...
const SiteInfoExtender = ( { site } ) => {
	...
	const saveCustomConfig = () => {
		...
		ipcRenderer.send('update-custom-config', 
			site.id,
			site.path,
			rootPath,
			imageURL,
	...
	const saveMultiSiteConfig = () => {
		...
		ipcRenderer.send('update-multisite-config',
			site.id,
			site.path,
		);
	...
	return (
		<>
			...
			<TableListRow key="remote-images" label="Remote Image URL">
				...
				/>
					<Text size="caption">Note: This will throw a 502 error if the url is invalid.</Text>
			</TableListRow>
