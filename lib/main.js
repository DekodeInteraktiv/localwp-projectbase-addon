...
const rewriteCafileEntry = (original, caBundlePath) => {
...
const updatePhpConfigs = (fileSystem, phpIniPaths, caBundlePath, localLogger) => __awaiter(void 0, void 0, void 0, function* () {
    let changed = false;
    const errors = [];
    for (const iniPath of phpIniPaths) {
        try {
            const original = yield fileSystem.readFile(iniPath, 'utf8');
            const { content, changed: fileChanged } = rewriteCafileEntry(original, caBundlePath);
            if (fileChanged) {
...
function default_1(context) {
    ...
    ipcMain.on('fix-ssl-config', (event, siteId, sitePath) => __awaiter(this, void 0, void 0, function* () {
        ...
        const wpCaBundlePath = path.join(resolvedSitePath, 'public', 'wp', 'wp-includes', 'certificates', 'ca-bundle.crt');
        const { changed, errors } = yield updatePhpConfigs(fileSystem, phpIniPaths, wpCaBundlePath, localLogger);
        if (errors.length) {
            context.notifier.notify({
                title: 'SSL config update failed',
