...
const SiteInfoExtender = ({ site }) => {
    const [state, setState] = (0, react_1.useReducer)((s, a) => (Object.assign(Object.assign({}, s), a)), {
        rootPath: '{{root}}',
    ...
    const convertToMultiSite = () => {
        setState({ updating: true });
        electron_1.ipcRenderer.send('update-multisite-config', site.id, site.path);
        electron_1.ipcRenderer.send('convert-to-multisite', site.id);
        const timeout = setTimeout(() => {
            setState({ updating: false });
        }, 2000);
    };
    const fixSslConfig = () => {
        setState({ updating: true });
        electron_1.ipcRenderer.send('fix-ssl-config', site.id, site.path);
        const timeout = setTimeout(() => {
            setState({ updating: false });
        }, 2000);
    };
    return (react_1.default.createElement(react_1.default.Fragment, null,
        site.multiSite && (react_1.default.createElement(local_components_1.TableListRow, { key: 'fixmultisite', label: ' ' },
        ...
        react_1.default.createElement(local_components_1.TableListRow, { key: 'fixssl', label: ' ' },
            react_1.default.createElement(local_components_1.TextButton, { disabled: updating ? 'true' : '', style: { paddingLeft: 0, float: 'right' }, onClick: fixSslConfig }, "Fix SSL config")),
        !(site === null || site === void 0 ? void 0 : site.multiSite) && (react_1.default.createElement(local_components_1.TableListRow, { key: 'convertMultisite', label: ' ' },
            react_1.default.createElement(local_components_1.TextButton, { disabled: updating ? 'true' : '', style: { paddingLeft: 0, float: 'right' }, onClick: convertToMultiSite }, "Convert to multisite (subdir)"))),
